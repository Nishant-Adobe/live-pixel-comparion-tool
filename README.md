...
Local comparison tool for opening two URLs side by side, running either a DOM-based pixel audit or a screenshot diff at common viewport sizes, and launching a live inspection session with mismatch overlays injected into both pages when DOM mode is available.
...
- Opens live Chromium windows with red and blue highlights over mismatched regions.
...
2. Pick a viewport preset.
3. Choose `DOM audit` or `Screenshot diff`.
4. Optionally provide a Playwright `storageState` JSON path if the site requires a legitimate logged-in session.
5. Run comparison.
6. Review the mismatch list or diff images.
7. In `DOM audit` mode, click `Open live inspection` to open both pages with overlays injected directly into the DOM.
...
5. Return to TwinPixel and click `Save session`.
6. TwinPixel fills the `Storage state JSON` field automatically.
